...
(function () {
  ...
  /** "YYYY-MM-DD" -> Excel 日期序列号；非日期原样返回（保证 Excel 公式能算日期） */
  function excelDate(str) {
    var m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(str || '');
    if (!m) return str;
    return Math.round(Date.UTC(+m[1], +m[2] - 1, +m[3]) / 86400000) + 25569;
  }
  /** 给 aoa 生成的 sheet 里若干列的数字单元格套上日期格式 */
  function applyDateFormats(ws, rowStart, rowEnd, cols) {
    for (var r = rowStart; r <= rowEnd; r++) {
      cols.forEach(function (c) {
        var cell = ws[XLSX.utils.encode_cell({ r: r, c: c })];
        if (cell && typeof cell.v === 'number') cell.z = 'yyyy-m-d';
      });
    }
  }
  ...
  /* ================= Excel 生成（下载完整版：房源/收款记录/收款提醒/证件存档/说明） ================= */
  function fcell(type, formula, z) {
    var c = { t: type, f: formula.charAt(0) === '=' ? formula.slice(1) : formula };
    if (z) c.z = z;
    return c;
  }
  function enc(r0, c0) { return XLSX.utils.encode_cell({ r: r0, c: c0 }); }

  function buildXlsx(data) {
    var wb = XLSX.utils.book_new();
    data = data || { houses: [], payments: [] };
    ...
    data.houses.forEach(function (h) {
      ...
      hr.push([
        ...
        c ? excelDate(c.startDate) : '', c ? excelDate(c.endDate) : '',
        c ? c.remark : ''
      ]);
    });
    var wsH = XLSX.utils.aoa_to_sheet(hr);
    applyDateFormats(wsH, 1, data.houses.length, [9, 10]);
    wsH['!cols'] = [{ wch: 22 }, { wch: 32 }, { wch: 8 }, { wch: 18 }, { wch: 14 }, { wch: 12 }, { wch: 12 },
                    { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 22 }];
    XLSX.utils.book_append_sheet(wb, wsH, '房源');

    /* ---- 2. 收款记录 ---- */
    var pr = [['收款日期', '房源名称', '类型', '金额(元)', '收款方式', '备注']];
    var nameOf = {};
    data.houses.forEach(function (h) { nameOf[h.id] = h.name; });
    data.payments.forEach(function (p) {
      pr.push([
        excelDate(p.payDate), nameOf[p.houseId] || '', TYPE_CN[p.type] || p.type,
        p.amount, METHOD_CN[p.method] || p.method, p.remark || ''
      ]);
    });
    var wsP = XLSX.utils.aoa_to_sheet(pr);
    applyDateFormats(wsP, 1, data.payments.length, [0]);
    wsP['!cols'] = [{ wch: 14 }, { wch: 22 }, { wch: 8 }, { wch: 12 }, { wch: 12 }, { wch: 20 }];
    XLSX.utils.book_append_sheet(wb, wsP, '收款记录');

    /* ---- 3. 收款提醒（公式自动算） ---- */
    var rs = {};
    rs[enc(0, 0)] = { t: 's', v: '收款提醒（按付款周期自动计算，打开文件自动更新，无需手动填写）' };
    rs[enc(1, 0)] = { t: 's', v: '今天日期：' };
    rs[enc(1, 1)] = fcell('n', 'TODAY()', 'yyyy-m-d');
    var sumLabels = ['出租中房源月租金合计', '本月实收房租（收款记录）', '已逾期未收合计'];
    var sumF = [
      'SUMIF(房源!$C$2:$C$101,"已出租",房源!$F$2:$F$101)',
      'SUMPRODUCT((收款记录!$A$2:$A$200>=DATE(YEAR(TODAY()),MONTH(TODAY()),1))*(收款记录!$A$2:$A$200<=EOMONTH(TODAY(),0))*(收款记录!$C$2:$C$200="房租")*收款记录!$D$2:$D$200)',
      'SUMPRODUCT(($H$9:$H$108="已逾期")*($C$9:$C$108))'
    ];
    sumLabels.forEach(function (lb, i) {
      rs[enc(3 + i, 0)] = { t: 's', v: lb };
      rs[enc(3 + i, 1)] = fcell('n', sumF[i], '#,##0');
    });
    var rsHdr = ['房源名称', '租客', '月租金', '付款周期', '上次房租收款日', '下次应缴日',
      '距下次应缴天数', '收租状态', '合同结束', '合同状态'];
    rsHdr.forEach(function (h, ci) { rs[enc(7, ci)] = { t: 's', v: h }; });

    for (var r0 = 8; r0 <= 107; r0++) {
      var r = r0 + 1;   // Excel 1-based 行 9..108
      var s = r - 7;    // 房源 1-based 行 2..101
      var rowCols = [
        { t: 's', f: 'IF(OR(房源!$A' + s + '="",房源!$C' + s + '<>"已出租"),"",房源!$A' + s + ')' },
        { t: 's', f: 'IF($A' + r + '="","",房源!$D' + s + ')' },
        { t: 'n', f: 'IF($A' + r + '="",0,房源!$F' + s + ')', z: '#,##0' },
        { t: 's', f: 'IF($A' + r + '="","",房源!$I' + s + ')' },
        { t: 'n', f: 'IF($A' + r + '="",0,MAXIFS(收款记录!$A$2:$A$200,收款记录!$B$2:$B$200,$A' + r + ',收款记录!$C$2:$C$200,"房租"))', z: 'yyyy-m-d' },
        { t: 'n', f: 'IF($A' + r + '="","",IF($E' + r + '>0,EDATE($E' + r + ',$L' + r + '),$M' + r + '))', z: 'yyyy-m-d' },
        { t: 'n', f: 'IF($A' + r + '="","",$F' + r + '-TODAY())', z: '0' },
        { t: 's', f: 'IF($A' + r + '="","",IF($F' + r + '<TODAY(),"已逾期",IF($F' + r + '-TODAY()<=3,"即将到期",IF($E' + r + '>0,"已缴清","待收"))))' },
        { t: 'n', f: 'IF($A' + r + '="","",房源!$K' + s + ')', z: 'yyyy-m-d' },
        { t: 's', f: 'IF($A' + r + '="","",IF(房源!$K' + s + '="","",IF(房源!$K' + s + '<TODAY(),"已到期",IF(房源!$K' + s + '-TODAY()<=30,"即将到期","正常"))))' }
      ];
      rowCols.forEach(function (cell, ci) {
        var c = { t: cell.t, f: cell.f };
        if (cell.z) c.z = cell.z;
        rs[enc(r0, ci)] = c;
      });
      // 辅助列 L(11)、M(12)
      rs[enc(r0, 11)] = fcell('n', 'IF($A' + r + '="",0,IF(房源!$I' + s + '="每3个月",3,IF(房源!$I' + s + '="每6个月",6,IF(房源!$I' + s + '="每12个月",12,1))))');
      rs[enc(r0, 12)] = fcell('n', 'IF($A' + r + '="","",DATE(YEAR(EDATE(房源!$J' + s + ',IF(DAY(房源!$J' + s + ')>MIN(房源!$H' + s + ',DAY(EOMONTH(房源!$J' + s + ',0))),1,0))),MONTH(EDATE(房源!$J' + s + ',IF(DAY(房源!$J' + s + ')>MIN(房源!$H' + s + ',DAY(EOMONTH(房源!$J' + s + ',0))),1,0))),MIN(房源!$H' + s + ',DAY(EOMONTH(EDATE(房源!$J' + s + ',IF(DAY(房源!$J' + s + ')>MIN(房源!$H' + s + ',DAY(EOMONTH(房源!$J' + s + ',0))),1,0)),0)))))', 'yyyy-m-d');
    }
    rs['!ref'] = 'A1:M108';
    rs['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 9 } }];
    rs['!cols'] = [{ wch: 22 }, { wch: 10 }, { wch: 10 }, { wch: 12 }, { wch: 16 }, { wch: 14 },
                   { wch: 14 }, { wch: 12 }, { wch: 14 }, { wch: 12 },
                   { wch: 2 }, { wch: 2 }, { wch: 2 }];
    XLSX.utils.book_append_sheet(wb, rs, '收款提醒');

    /* ---- 4. 证件存档 ---- */
    var acRows = [['房源名称', '证件类型', '扫描件（在这里插入图片）', '存档日期', '备注']];
    data.houses.forEach(function (h) {
      acRows.push([h.name, '', '', '', '']);
    });
    var wsA = XLSX.utils.aoa_to_sheet(acRows);
    wsA['!cols'] = [{ wch: 22 }, { wch: 14 }, { wch: 40 }, { wch: 12 }, { wch: 20 }];
    var rowsInfo = [{ hpt: 20 }];
    for (var ri = 0; ri < data.houses.length; ri++) rowsInfo.push({ hpt: 150 });
    wsA['!rows'] = rowsInfo;
    XLSX.utils.book_append_sheet(wb, wsA, '证件存档');

    /* ---- 5. 说明 ---- */
    var descLines = [
      ['租房管家 · 使用说明（下载版）'],
      [''],
      ['· 【房源】【收款记录】为云端同步的数据，直接编辑即可。'],
      ['· 【收款提醒】自动计算：最近一次房租收款日 + 付款周期 = 下次应缴日；距应缴≤3天未收=即将到期，已过=已逾期，本周期已收=已缴清。'],
      ['· 【证件存档】放房产证/合同扫描件图片（插入→图片→拖到扫描件列）。图片只随本文件保存，不会同步到网页云端。'],
      ['· 编辑后请重新上传到网页，让云端保持最新。'],
      ['· 请勿改动【收款提醒】的公式区。']
    ];
    var wsD = XLSX.utils.aoa_to_sheet(descLines);
    wsD['!cols'] = [{ wch: 100 }];
    XLSX.utils.book_append_sheet(wb, wsD, '说明');
  ...
  /** 本月应收清单：本月应缴 + 已逾期的房源（已收的不列） */
  function computeMonthly(data, now) {
    var today = utils.fmtDate(now);
    var mk = utils.monthKeyOf(now);
    var y = now.getFullYear(), m = now.getMonth() + 1;
    var monthEnd = y + '-' + utils.pad2(m) + '-' + utils.pad2(utils.lastDayOfMonth(y, m));
    var items = [];
    (data.houses || []).forEach(function (h) {
      if (h.status !== 'rented' || !h.contract) return;
      var c = h.contract;
      ...
      else if (nextDue <= monthEnd) status = 'due';
      else return; // 本月不收，不列入
      items.push({ house: h, due: nextDue, amount: c.monthlyRent || 0, status: status });
    });
    items.sort(function (a, b) { return a.due < b.due ? -1 : (a.due > b.due ? 1 : 0); });
    var totalDue = items.reduce(function (s, it) { return s + it.amount; }, 0);
    var received = (data.payments || []).filter(function (p) {
      return p.type === 'rent' && p.payDate && p.payDate.indexOf(mk) === 0;
    }).reduce(function (s, p) { return s + p.amount; }, 0);
    return { items: items, totalDue: totalDue, received: received };
  }

  function renderHome() {
    var now = new Date();
    var mo = computeMonthly(state, now);
    var reminders = computeReminders(state, now).filter(function (r) {
      return r.type === 'contract_expired' || r.type === 'contract_expiring';
    });
    var rented = (state.houses || []).filter(function (h) { return h.status === 'rented'; }).length;
    var vacant = (state.houses || []).length - rented;
    var shortMoney = function (n) { return '¥' + utils.fmtMoney(n).replace(/\.00$/, ''); };

    var html = '';
    html += '<div class="stats-grid">' +
      stat('primary', shortMoney(mo.totalDue), '本月应收') +
      stat('green', shortMoney(mo.received), '本月已收') +
      stat('', rented, '出租中') +
      stat('orange', vacant, '空置') +
      '</div>';

    html += '<div class="section-label">本月应收清单（' + (now.getMonth() + 1) + '月）</div>';
    if (!mo.items.length) {
      html += '<div class="card empty-state"><span class="emoji">😌</span>本月暂无需收租的房源</div>';
    } else {
      html += '<div class="table-wrap"><table class="table"><thead><tr>' +
        '<th>应缴日</th><th>房源</th><th>租客</th><th>本次应缴</th><th>状态</th></tr></thead><tbody>';
      mo.items.forEach(function (it) {
        var c = it.house.contract;
        var st = it.status === 'overdue'
          ? '<span class="badge" style="background:#fff1f0;color:#f5222d">已逾期' + diffDaysStr(it.due, utils.fmtDate(now)) + '天</span>'
          : '<span class="badge" style="background:#fff7e6;color:#fa8c16">待收</span>';
        html += '<tr>' +
          '<td>' + utils.fmtDateCN(it.due) + '</td>' +
          '<td>' + esc(it.house.name) + '</td>' +
          '<td>' + esc(c ? c.tenantName : '') + '</td>' +
          '<td>¥' + utils.fmtMoney(it.amount) + '</td>' +
          '<td>' + st + '</td>' +
          '</tr>';
      });
      html += '</tbody></table></div>';
      html += '<div class="card" style="margin-top:10px;text-align:center"><b>本月应收合计：' + shortMoney(mo.totalDue) + '</b>　共 ' + mo.items.length + ' 套</div>';
    }

    if (reminders.length) {
      html += '<div class="section-label">合同提醒</div>';
      reminders.forEach(function (r) {
        var color = (r.type === 'contract_expired') ? 'danger' : 'blue';
  ...
  function renderHouses() {
    ...
    html += '<div class="table-wrap"><table class="table">' +
      '<thead><tr><th>房源</th><th>状态</th><th>租客</th><th>每次收租</th><th>收租日</th><th>周期</th><th>合同至</th></tr></thead><tbody>';
